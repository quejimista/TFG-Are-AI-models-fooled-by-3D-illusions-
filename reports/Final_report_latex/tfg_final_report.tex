...
\subsection{From CNNs to Vision Foundation Models}

Early Convolutional Neural Networks (CNNs) extracted features at local scales, sometimes
missing global scene coherence because they prioritise local pixel regions over the whole
image. Vision Transformers (ViTs) introduced self-attention mechanisms, allowing the
model to weigh the importance of different parts of an image simultaneously regardless of
their distance, which enables a more global understanding of the scene.
MiDaS~\cite{ranftl2020} extended CNN-based depth estimation to multi-dataset training;
the Depth Anything family (V1, V2, V3) further scales this with ViT backbones and
semi-supervised training on large unlabelled corpora~\cite{depthanythingv1,
...
depth perception. Fifteen controlled variants are generated across five sub-categories:
...
depth (darker pixels) to the geometrically \emph{higher} object in the Corridor and Ponzo
illusions, mirroring the human tendency to interpret height-in-field as distance. V3
...
dominant interpretation (light from above). V1 fails to produce a coherent Necker's Cube
...
Room and Beuchet's Chair, all three versions correctly identify that geometrically separate
...
\textbf{Height Bias .} A square slides from top to bottom across
...
object be treated as the closer one. 
...
cue in a broadly similar way, in contrast to V1's rigid response.
...
from both of its predecessors rather than simply being a refined version of either.
...
than the other geometric-cue experiments.
...
(127.8 to 185.1), similarly to V1, never reaching subjective equality within this range.
...
sensitivity to the shadow as a grounding cue.
...
neighbours, rather than simply weaker or stronger.
...
in sign with V1 and V3, mirroring the pattern seen for Kanizsa.
...
\begin{figure}[H]
  \centering
  % Asumo que la imagen se llama fig_ponzo_bias, cámbialo si es distinto
...
default positional prior on top of it. The same gradient dominates the Ponzo and
...
incongruence, anamorphic cube) give MiDaS the lowest average $\Delta I$ of any
...
This work demonstrates that modern monocular depth estimation models are systematically vulnerable to 3D optical illusions, revealing consistent and measurable biases across architectures.

The results indicate that different model families develop distinct  priors that influence depth perception, with notable differences between convolutional and transformer-based systems.

Furthermore, architectural design plays a key role in shaping the trade-off between semantic understanding and geometric robustness.

Finally, the study highlights that these biases are structural rather than data-dependent, as they persist across synthetic voxel environments. \newline
...
There are several limitations that must be acknowledged. The first one is the dataset; since it has been curated to maintain diversity, it doesn't reflect the frequency of these ambiguities in the real world. Another limitation is that although the ROI extraction uses the mean of the intensity of the region, mitigating  noise and possible artifacts, the area is extracted by a human, introducing a small amount of subjectivity and bias. \newline

\noindent Moreover, only one CNN-based model has been analyzed; to generalize the behavior of CNN models against 3D visual illusions, it would be wise to test additional models. \newline
 
\noindent Finally, due to hardware limitations, the evaluation of the Depth Anything family and MiDaS was restricted to the "Base" variants. Larger variants could possess a greater representational capacity and may have different biases.
