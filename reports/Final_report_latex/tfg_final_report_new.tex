...
\begin{figure}[H]
  ...
  \caption{Cross-domain validation of the Fake Block stimulus processed by MiDaS. Reflecting a low average contextual sensitivity ($\Delta I = 7.9$ for forced perspective), the output confirms that the model's resistance to scale anomalies is an architecture-level property preserved under domain shifts[cite: 1].}
...
\begin{figure}[H]
  ...
  \caption{Cross-domain validation of the Depth Incongruence experiment under MiDaS. The resulting depth map captures the model's inherent resistance to perspective context, failing to be deceived by the size-distance manipulation and relies instead on its rigid vertical mapping[cite: 1].}
